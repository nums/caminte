 ...
 */
function diff(d1, d2, props) {
    let data = {};
    for(var i in d1) {
        if(props[i].type.name == 'JSON') {
            if(JSON.stringify(d1[i]) != JSON.stringify(d2[i])) {
                //console.log(i, 'diff', props[i].type.name, d1[i], d2[i])
                data[i] = d1[i];
            }
        }
        else {
        ...
        this.trigger('save', function (saveDone) {
            ...
            if (inst.id) {
                ...
                inst.trigger('update', function (updateDone) {
                    //inst._adapter().oldData = this.oldData;
                    
                    let newData;
                    if(this.oldData) {
                        newData = diff(data, JSON.parse(this.oldData), this.constructor.schema.definitions[modelName].properties)
                    ...
                    inst._adapter().save(modelName, inst.constructor._forDB(newData), function (err) {
                        if (err) {
                            console.log(err);
                        } else {
...
AbstractClass.update = function update(filter, data, options, callback) {
    ...
    if (this.schema.adapter.update) {
        var args = [this.modelName, filter, this._forDB(data)];
